    ...
    <div class="gva-form-box">
      <el-form :model="formData" ref="elFormRef" label-position="right" :rules="rule" label-width="80px">
        ...
        <el-form-item label="主体信息:" prop="subjectInfo">
          <el-input v-model="formData.subjectInfo" :clearable="true" placeholder="请输入" />
       </el-form-item>
        <el-form-item label="营业执照注册号:" prop="license">
          <el-input v-model="formData.license" :clearable="true" placeholder="请输入" />
       </el-form-item>
...
const init = async () => {
 // 建议通过url传参获取目标数据ID 调用 find方法进行查询数据操作 从而决定本页面是create还是update 以下为id作为url参数示例
    if (route.query.id) {
      const res = await findAdvertiser({ ID: route.query.id })
      if (res.code === 0) {
