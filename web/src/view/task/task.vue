<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="任务类型" prop="category">
             ...
             <el-input v-model.number="searchInfo.category" placeholder="搜索条件" />
        ...
        <el-form-item label="任务校区" prop="campus">
         <el-input v-model="searchInfo.campus" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="任务学院" prop="college">
         <el-input v-model="searchInfo.college" placeholder="搜索条件" />

        </el-form-item>
            <el-form-item label="是否需要完成主线任务" prop="needMain">
            <el-select v-model="searchInfo.needMain" clearable placeholder="请选择">
                <el-option
                    key="true"
                    label="是"
                    value="true">
        ...
        <el-form-item label="开始时间" prop="startTime">
            ...
            <el-date-picker v-model="searchInfo.startStartTime" type="datetime" placeholder="开始日期" :disabled-date="time=> searchInfo.endStartTime ? time.getTime() > searchInfo.endStartTime.getTime() : false"></el-date-picker>
            —
            <el-date-picker v-model="searchInfo.endStartTime" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startStartTime ? time.getTime() < searchInfo.startStartTime.getTime() : false"></el-date-picker>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="任务类型" prop="category" width="120" />
        <el-table-column align="left" label="任务名称" prop="title" width="120" />
        <el-table-column align="left" label="任务描述" prop="desc" width="120" />
        <el-table-column align="left" label="任务校区" prop="campus" width="120" />
        <el-table-column align="left" label="任务学院" prop="college" width="120" />
        <el-table-column align="left" label="奖励积分" prop="reward" width="120" />
        <el-table-column align="left" label="是否需要完成主线任务" prop="needMain" width="120">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateTaskFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="type==='create'?'添加':'修改'" destroy-on-close>
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="任务类型:"  prop="category" >
              <el-input v-model.number="formData.category" :clearable="true" placeholder="请输入任务类型" />
            </el-form-item>
            <el-form-item label="任务名称:"  prop="title" >
              <el-input v-model="formData.title" :clearable="true"  placeholder="请输入任务名称" />
            </el-form-item>
            <el-form-item label="任务描述:"  prop="desc" >
              <el-input v-model="formData.desc" :clearable="true"  placeholder="请输入任务描述" />
            </el-form-item>
            <el-form-item label="任务校区:"  prop="campus" >
              <el-input v-model="formData.campus" :clearable="true"  placeholder="请输入任务校区" />
            </el-form-item>
            <el-form-item label="任务学院:"  prop="college" >
              <el-input v-model="formData.college" :clearable="true"  placeholder="请输入任务学院" />
            </el-form-item>
            <el-form-item label="奖励积分:"  prop="reward" >
              <el-input v-model.number="formData.reward" :clearable="true" placeholder="请输入奖励积分" />
            </el-form-item>
            <el-form-item label="是否需要完成主线任务:"  prop="needMain" >
  ...
  createdAt: [
    { validator: (rule, value, callback) => {
      if (searchInfo.value.startCreatedAt && !searchInfo.value.endCreatedAt) {
        ...
        if (searchInfo.value.startStartTime && !searchInfo.value.endStartTime) {
          callback(new Error('请填写结束日期'))
        } else if (!searchInfo.value.startStartTime && searchInfo.value.endStartTime) {
          callback(new Error('请填写开始日期'))
        } else if (searchInfo.value.startStartTime && searchInfo.value.endStartTime && (searchInfo.value.startStartTime.getTime() === searchInfo.value.endStartTime.getTime() || searchInfo.value.startStartTime.getTime() > searchInfo.value.endStartTime.getTime())) {
          callback(new Error('开始日期应当早于结束日期'))
        } else {
          callback()
        }
